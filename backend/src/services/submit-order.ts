...
export function parseOrderSubmission(value: unknown): OrderSubmission {
  ...
  const customerName = requiredString(input, 'customerName', 'Customer name', 120, errors);
  const phoneNumber = requiredString(input, 'phoneNumber', 'Phone number', 30, errors);
  const emailAddress = requiredString(input, 'emailAddress', 'Email address', 254, errors);
  const bikeBrand = requiredString(input, 'bikeBrand', 'Bike brand', 120, errors);
  ...
  if (phoneNumber && (!/^\+?[\d ()-]+$/.test(phoneNumber) || phoneDigits.length < 7)) {
  ...
  if (!Array.isArray(rawServiceTypes) || rawServiceTypes.length === 0) {
    ...
    rawServiceTypes.some((serviceType) => typeof serviceType !== 'string' || !isServiceTypeCode(serviceType))
  ...
  return { customerName, phoneNumber, emailAddress, bikeBrand, serviceTypes, notes };
}
...
async function findEarliestReservation(
  ...
  for (let day = 0; day < MAX_SCHEDULING_DAYS; day += 1) {
    if (!isWeekend(dueDate)) {
      ...
      const slot = CAPACITY_SLOT_NUMBERS.find((candidate) => !usedSlots.has(candidate));
...
export async function submitOrder(
  ...
  const firstDate = addCalendarDays(shopLocalCalendarDate(now(), shopTimeZone), 1);

  for (let attempt = 0; attempt < MAX_TRANSACTION_ATTEMPTS; attempt += 1) {
    const reference = generateReference();

    try {
      const createdOrder = await client.$transaction(async (transaction) => {
        const { dueDate, slot } = await findEarliestReservation(transaction, firstDate);
      ...
      return {
        ...
        serviceTypes: input.serviceTypes.map((code) => {
          const serviceType = SERVICE_TYPES.find((candidate) => candidate.code === code)!;
          return { ...serviceType };
        }),
      };
    } catch (error) {
