...
function submissionClient(reservationsByDate: Record<string, number[]> = {}) {
  const create = vi.fn(async ({ data }: { data: { expectedDueDate: string } }) => ({
    id: 'order-1',
...
describe('order submission validation', () => {
  ...
  it.each([
    ...
    expect(() => parseOrderSubmission({ ...validInput, [field]: invalidValue }))
  ...
  it('retries the transaction with a new reference after a unique collision', async () => {
    ...
    const client = {
      $transaction: vi.fn()
        ...
        .mockImplementation((operation) => workingClient.$transaction(operation)),
    } as unknown as Pick<PrismaClient, '$transaction'>;
...
describe('submission helpers', () => {
  ...
  it('uses the configured shop timezone for the local calendar day', () => {
    ...
    expect(shopLocalCalendarDate(instant, 'America/New_York')).toBe('2026-08-22');
  });

  it('rejects an invalid configured shop timezone', () => {
    expect(() => assertValidShopTimeZone('Not/A_Timezone')).toThrow(/SHOP_TIME_ZONE/);
  });
});
