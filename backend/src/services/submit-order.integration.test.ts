...
beforeAll(async () => {
  temporaryDirectory = await mkdtemp(
    path.join(tmpdir(), "bouvet-bike-submission-"),
  );
...
describe("submitOrder with SQLite", () => {
  it("keeps six concurrent submissions within weekday capacity", async () => {
    ...
    expect(
      orders.filter(({ expectedDueDate }) => expectedDueDate === "2026-08-24"),
    ).toHaveLength(5);
    expect(
      orders.filter(({ expectedDueDate }) => expectedDueDate === "2026-08-25"),
    ).toHaveLength(1);
  ...
  it("rolls back the order when a nested service write fails", async () => {
    ...
    await expect(
      submitOrder(input(8, ["CHAIN_REPLACEMENT"]), {
        client,
    ...
    await expect(
      client.order.findUnique({ where: { reference: "ROLLBACK" } }),
    ).resolves.toBeNull();
  });
});
